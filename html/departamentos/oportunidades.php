<!DOCTYPE html>
<html lang="es">
<head>
	<?php include 'http://mortonsubastas.andyosuna.com/php/include-meta.php'; ?>
	
	<title>Morton Casa de Subastas - Oportunidades: Artes Decorativas</title>

	<?php include 'http://mortonsubastas.andyosuna.com/php/include-css.php'; ?>
</head>

<body>
	<?php include 'http://mortonsubastas.andyosuna.com/php/include-header.php'; ?>

	<section id="banner">
		<img src="/images/banners/dep-arte-moderno.jpg">
	</section>

	<div id="content">

		<h1>Oportunidades</h1>

		<ol class="breadcrumb">
			<li><a href="/">Inicio</a></li>
			<li><a href="/departamentos/">Departamentos</a></li>
			<li class="active">Oportunidades</li>
		</ol>

		<section class="cont">

			<h2 class="align-left">
				Departamento Artes Decorativas Moliere<br>
				<small>Artes Decorativas y Dise&ntilde;o. De las vanguardias a lo contempor&aacute;neo.</small>
			</h2>

			<img src="/images/cont-sample.jpg" width="306" height="198" class="pull-right">
			<p>Cada s&aacute;bado en nuestro Sal&oacute;n Moliere gran cantidad de piezas art&iacute;sticas y enseres decorativos, se programan para ser subastados profesionalmente bajo un ambiente seguro, confiable, honesto y transparente. Nuestras subastas conjugan de forma singular la posibilidad de colocar atractivamente en mercado las piezas que se deseen vender en tanto que al sector de compradores les brinda la opci&oacute;n de acercarse a un universo de objetos selectos para su disfrute, inversi&oacute;n y uso.</p>
			<p>Nuestra divisi&oacute;n contempla principalmente la consignaci&oacute;n y venta de objetos como cristaler&iacute;as, porcelanas, plater&iacute;a, mobiliario, pinturas, esculturas, alfombras, l&aacute;mparas y candiles, entre otros.</p>

		</section>

		<hr>

		<section class="cont clearfix">

			<h2 class="align-left">Especialistas</h2>

			<div class="staff">
				<div class="staff-box">
					<img src="/images/staff/ortegon-hanzel.jpg" width="153" height="200">
					<h3>Hanzel Orteg&oacute;n G&oacute;mez</h3>
					<p>Hanzel Orteg&oacute;n, curs&oacute; estudios de Comunicaci&oacute;n en la Universidad Iberoamericana, realiz&oacute; diversos diplomados en torno al periodo hisp&aacute;nico y la Maestr&iacute;a en Cultura Virreinal en la Universidad del Claustro de Sor Juana. Su &aacute;rea de especializaci&oacute;n corresponde a las manifestaciones art&iacute;sticas chino-filipinas durante el siglo XVIII. Su experiencia profesional se ha centrado en el medio museal y mercado de arte trabajando en instituciones como Fomento Cultural Banamex, Museo Franz Mayer y desde hace cinco a&ntilde;os en Morton, Casa de Subastas.  Ha sido expositor de tem&aacute;ticas virreinales en el programa acad&eacute;mico del Museo Nacional del Virreinato y participado como conferencista en tem&aacute;ticas de mercado de arte en el Instituto Cultural Hel&eacute;nico y diversas universidades.</p>
				</div><!-- end .staff-box -->
			</div><!-- end .staff -->

			<div class="staff">
				<div class="staff-box">
					<img src="/images/staff/renaud-eduardo.jpg" width="153" height="200">
					<h3>Eduardo Renaud Ibarra</h3>
					<h4>Valuador de Artes Decorativas y Dise&ntilde;o</h4>
					<p>Eduardo Renaud, es egresado en estudios de Historia del Arte por la Universidad Iberoamericana. Realiz&oacute; estudios sobre gesti&oacute;n de la cultura, patrimonio y mercado de arte. Su &aacute;rea de especialidad son las producciones art&iacute;sticas virreinales y tendencias de las Vanguardias Art&iacute;sticas en Europa. Su experiencia en el campo profesional se ha desarrollado en instituciones museales, galer&iacute;as de arte y en el sector de investigaci&oacute;n. Actualmente desempe&ntilde;a funciones en Morton, Casa de Subastas como valuador del Departamento de Artes Decorativas y Dise&ntilde;o. Ha participado en diversos programas de investigaci&oacute;n a nivel nacional e internacional, as&iacute; como impartido ponencias sobre mercado de Arte y estudios de caso, de las manifestaciones art&iacute;sticas de los siglos XIX y XX.</p>
				</div><!-- end .staff-box -->
			</div><!-- end .staff -->

			<div class="staff">
				<div class="staff-box">
					<img src="/images/staff/loin-pork.jpg" width="153" height="200">
					<h3>Leonardo Medina L&oacute;pez</h3>
					<h4>Valuador de Artes Decorativas y Dise&ntilde;o</h4>
					<p>Leonardo Medina, curs&oacute; estudios de Licenciatura en Econom&iacute;a en la Universidad Veracruzana. Desempe&ntilde;&oacute; funciones de catalogaci&oacute;n y archivo de acervo social y antropol&oacute;gico en CIESAS-Golfo. Desde el a&ntilde;o 2002, se ha desempe&ntilde;ado como valuador en el Departamento de Artes Decorativas y Dise&ntilde;o en Morton Casa de Subastas.</p>
				</div><!-- end .staff-box -->
			</div><!-- end .staff -->

		</section><!-- end .cont -->

	</div><!-- end #content -->

	<?php include 'http://mortonsubastas.andyosuna.com/php/include-footer.php'; ?>

    <?php include 'http://mortonsubastas.andyosuna.com/php/include-js.php'; ?>
</body>
</html>